...
function createTournamentImage(obj){
...
function handleCanvas(tournament){
    ...
    for (let i = 0; i < iterations; i++){
        ...
        console.log(tournament[i].length)
        for (let j = 0; j < tournament[i].length; j += 2){
            let bracket = document.createElement("div");

            bracket.classList.add("bracket");
            bracket.style.height = `${bracketSize / 2}px`
            bracket.appendChild(createTournamentImage(tournament[i][j]));
            if (tournament[i][j + 1] != undefined){
                bracket.appendChild(createTournamentImage(tournament[i][j + 1]));
                bracket.style.width = `${bracketSize}px`
            }
            else{
                bracket.appendChild(getCrown())
                bracket.style.width = `${bracket / 2}px`
                bracket.querySelector(".tournamentImageWrap").setAttribute("tournamentWinner", "true")
            }
